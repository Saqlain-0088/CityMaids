    ...
    <header className={`sticky top-0 z-50 transition-all duration-300 ${scrolled ? 'bg-white/95 backdrop-blur-md shadow-sm border-b border-slate-100' : 'bg-white border-b border-transparent'}`}>
      <div className="container-xl">
        <div className="flex items-center justify-between h-18 py-3">
          ...
          <nav className="hidden lg:flex items-center gap-1">
            {nav.map(item => (
              <Link key={item.href} to={item.href}
                className={`relative px-4 py-2 text-sm font-medium rounded-lg transition-colors ${isActive(item.href) ? 'text-primary-700 bg-primary-50' : 'text-slate-600 hover:text-slate-900 hover:bg-slate-50'}`}>
                {item.label}
                {isActive(item.href) && <span className="absolute bottom-0 left-4 right-4 h-0.5 bg-primary-500 rounded-full" />}
              </Link>
            ))}
          </nav>
          ...
          <div className="hidden lg:flex items-center gap-2">
            ...
            <Link to="/booking" className="btn-cta btn-md">
      ...
      <div className={`lg:hidden overflow-hidden transition-all duration-300 ${open ? 'max-h-screen' : 'max-h-0'}`}>
        <div className="border-t border-slate-100 bg-white px-4 py-4 space-y-1">
          {nav.map(item => (
            <Link key={item.href} to={item.href}
              className={`block px-4 py-3 rounded-xl text-sm font-medium transition-colors ${isActive(item.href) ? 'bg-primary-50 text-primary-700' : 'text-slate-700 hover:bg-slate-50'}`}>
              {item.label}
            </Link>
          ))}
          <div className="pt-3 flex flex-col gap-2 border-t border-slate-100 mt-2">
            ...
            <Link to="/booking" className="btn-cta btn-md w-full justify-center">Book Now</Link>
