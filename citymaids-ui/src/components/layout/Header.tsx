...
export default function Header() {
  ...
  return (
    <header className="sticky top-0 z-50">
      {/* Top info bar */}
      <div className="bg-primary-700 hidden lg:block">
      ...
      {/* Main nav */}
      <div className={`transition-all duration-300 ${scrolled ? 'bg-white/98 backdrop-blur-md shadow-md' : 'bg-white'} border-b border-slate-100`}>
        <div className="container-xl">
          <div className="flex items-center justify-between h-16">

            {/* Logo */}
            <Link to="/" className="flex items-center gap-3 group">
              <div className="w-10 h-10 rounded-xl bg-primary-600 flex items-center justify-center shadow-blue group-hover:bg-primary-700 transition-colors flex-shrink-0">
                <svg className="w-5 h-5 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" />
                </svg>
              </div>
              <div>
                <span className="block text-xl font-extrabold text-slate-900" style={{ fontFamily: 'Plus Jakarta Sans, sans-serif' }}>
            ...
            <nav className="hidden lg:flex items-center gap-0.5">
              {nav.map(item => (
                <Link key={item.href} to={item.href}
                  className={`relative px-4 py-2 text-sm font-semibold rounded-lg transition-all duration-150 ${isActive(item.href) ? 'text-primary-700 bg-primary-50' : 'text-slate-600 hover:text-primary-700 hover:bg-primary-50'}`}>
                  {item.label}
                  {isActive(item.href) && <span className="absolute bottom-0.5 left-4 right-4 h-0.5 bg-primary-500 rounded-full" />}
                </Link>
              ))}
            </nav>
            ...
            <div className="hidden lg:flex items-center gap-2">
              <a href="https://wa.me/15550001234?text=Hi%2C%20I%20want%20to%20book%20a%20cleaning%20service"
                ...
                className="flex items-center gap-1.5 text-sm font-semibold text-[#25D366] hover:text-[#20b858] transition-colors px-3 py-2 rounded-lg hover:bg-green-50">
                <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d={WA_D} /></svg>
                WhatsApp
              </a>
              <Link to="/login" className="text-sm font-semibold text-slate-600 hover:text-slate-900 px-3 py-2 rounded-lg hover:bg-slate-50 transition-colors">
                Sign In
              </Link>
              <Link to="/booking"
                className="inline-flex items-center gap-2 bg-primary-600 hover:bg-primary-700 text-white font-bold px-5 py-2.5 rounded-xl text-sm transition-all duration-200 shadow-blue hover:-translate-y-0.5">
