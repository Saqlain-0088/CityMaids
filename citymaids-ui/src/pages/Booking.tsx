...
export default function Booking() {
      ...
      <div className="min-h-[70vh] flex items-center justify-center px-4 py-20">
        <div className="max-w-md w-full text-center">
          <div className="w-24 h-24 rounded-full bg-brand-100 flex items-center justify-center mx-auto mb-6">
            <svg className="w-12 h-12 text-brand-600" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg>
          </div>
          <h2 className="text-3xl font-extrabold text-dark-900 mb-2" style={{ fontFamily: 'Plus Jakarta Sans, sans-serif' }}>Booking Confirmed!</h2>
          <p className="text-dark-500 mb-8">Confirmation sent to <strong className="text-dark-900">{form.email}</strong></p>
          <div className="bg-dark-50 rounded-2xl p-5 text-left mb-8 space-y-3 text-sm border border-dark-100">
            {[['Service', svc?.name], ['Date', form.date], ['Time', form.time], ['Address', form.address]].map(([l, v]) => (
              <div key={l} className="flex justify-between gap-4"><span className="text-dark-400">{l}</span><span className="font-semibold text-dark-900 text-right">{v}</span></div>
            ))}
            <div className="flex justify-between pt-2 border-t border-dark-200"><span className="font-bold text-dark-900">Total</span><span className="font-extrabold text-brand-600 text-lg">${svc?.price}</span></div>
          </div>
  ...
  return (
    <MainLayout>
      <div className="bg-gradient-to-br from-dark-900 to-dark-800 py-14">
        <div className="container-xl text-center">
          ...
          <p className="text-dark-300">Complete the steps below to schedule your service.</p>
        </div>
      </div>
          ...
          {STEPS.map((label, i) => (
            <div key={label} className="flex items-center flex-1">
              <div className="flex flex-col items-center">
                <div className={`w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold transition-all duration-300 ${i < step ? 'bg-brand-500 text-white' : i === step ? 'bg-brand-600 text-white shadow-brand' : 'bg-dark-100 text-dark-400'}`}>
                ...
                <span className={`text-xs mt-1.5 font-medium hidden sm:block ${i === step ? 'text-brand-600' : i < step ? 'text-brand-500' : 'text-dark-400'}`}>{label}</span>
              </div>
              {i < STEPS.length - 1 && <div className={`flex-1 h-0.5 mx-2 transition-all duration-500 ${i < step ? 'bg-brand-400' : 'bg-dark-200'}`} />}
            </div>
          ))}
        </div>

        <div className="bg-white rounded-3xl border border-dark-100 shadow-card p-6 sm:p-8">
          {step === 0 && (
            <div>
              <h2 className="text-xl font-bold text-dark-900 mb-6">Choose a Service</h2>
              <div className="space-y-3">
                {services.map(s => (
                  <label key={s.id} className={`flex items-center gap-4 p-4 rounded-2xl border-2 cursor-pointer transition-all duration-200 ${form.serviceId === String(s.id) ? 'border-brand-500 bg-brand-50' : 'border-dark-100 hover:border-dark-200 hover:bg-dark-50'}`}>
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="font-bold text-dark-900">{s.name}</p>
                      <p className="text-sm text-dark-500 truncate">{s.shortDescription}</p>
                    </div>
                    <div className="text-right flex-shrink-0">
                      <p className="font-extrabold text-brand-600 text-lg">${s.price}</p>
                      <p className="text-xs text-dark-400">{s.duration}h</p>
                    </div>
          ...
          {step === 1 && (
            <div>
              <h2 className="text-xl font-bold text-dark-900 mb-6">Choose Date & Time</h2>
              <div className="space-y-6">
                <Input label="Preferred Date" type="date" value={form.date} min={today} onChange={e => set('date', e.target.value)} required />
                <div>
                  <label className="block text-sm font-semibold text-dark-700 mb-3">Preferred Time <span className="text-red-500">*</span></label>
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                    {TIMES.map(t => (
                      <button key={t} type="button" onClick={() => set('time', t)}
                        className={`py-3 px-3 rounded-xl text-sm font-semibold border-2 transition-all duration-200 ${form.time === t ? 'border-brand-500 bg-brand-50 text-brand-700' : 'border-dark-100 text-dark-600 hover:border-dark-200 hover:bg-dark-50'}`}>
                        {t}
                      </button>
                    ))}
                  </div>
          ...
          {step === 2 && (
            <div>
              <h2 className="text-xl font-bold text-dark-900 mb-6">Your Details</h2>
          ...
          {step === 3 && (
            <div>
              <h2 className="text-xl font-bold text-dark-900 mb-6">Review Your Booking</h2>
              <div className="space-y-4">
                <div className="bg-brand-50 border border-brand-100 rounded-2xl p-5 flex items-center gap-4">
                  ...
                  <div className="flex-1"><p className="font-bold text-dark-900 text-lg">{svc?.name}</p><p className="text-sm text-dark-500">{svc?.duration} hours</p></div>
                  <p className="text-3xl font-extrabold text-brand-600">${svc?.price}</p>
                </div>
                <div className="bg-dark-50 rounded-2xl p-5 space-y-3 text-sm border border-dark-100">
                  {[['Date', form.date], ['Time', form.time], ['Name', form.name], ['Email', form.email], ['Phone', form.phone], ['Address', form.address]].map(([l, v]) => (
                    <div key={l} className="flex justify-between gap-4"><span className="text-dark-400 flex-shrink-0">{l}</span><span className="font-semibold text-dark-900 text-right">{v}</span></div>
                  ))}
                </div>
                <div className="flex items-center justify-between p-4 bg-brand-50 rounded-2xl border border-brand-100">
                  <span className="font-bold text-dark-900">Total Amount</span>
                  <span className="text-2xl font-extrabold text-brand-600">${svc?.price}</span>
                </div>
                <p className="text-xs text-dark-400 text-center">By confirming, you agree to our Terms of Service and Cancellation Policy.</p>
              </div>
            </div>
          )}

          <div className="flex justify-between mt-8 pt-6 border-t border-dark-100">
            <Button variant="ghost" className="border border-dark-200" onClick={() => setStep(s => s - 1)} disabled={step === 0} size="lg">← Back</Button>
            {step < STEPS.length - 1
              ? <Button onClick={handleNext} disabled={!canNext()} size="lg">Continue →</Button>
              : <Button onClick={() => setDone(true)} size="lg">Confirm Booking ✓</Button>}
          </div>
        </div>
      </div>
    </MainLayout>
