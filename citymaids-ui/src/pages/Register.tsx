...
export default function Register() {
  ...
  const strengthColor = ['', 'bg-red-400', 'bg-yellow-400', 'bg-blue-400', 'bg-brand-500']
    ...
    <div className="min-h-screen bg-mesh bg-dark-50 flex items-center justify-center px-4 py-12">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-extrabold text-dark-900" style={{ fontFamily: 'Plus Jakarta Sans, sans-serif' }}>Create your account</h1>
          <p className="text-dark-500 mt-1">Join thousands of happy customers</p>
        </div>

        <div className="bg-white rounded-3xl border border-dark-100 shadow-card p-7 space-y-4">
          ...
          <div>
            ...
            {form.password && (
              <div className="mt-2">
                ...
                <p className={`text-xs font-medium ${strength <= 1 ? 'text-red-500' : strength === 2 ? 'text-yellow-500' : strength === 3 ? 'text-blue-500' : 'text-brand-600'}`}>{strengthLabel[strength]} password</p>
              </div>
            )}
          </div>
          ...
          <p className="text-center text-sm text-dark-500">Already have an account? <Link to="/login" className="text-brand-600 font-semibold hover:text-brand-700 transition-colors">Sign in</Link></p>
        </div>
      </div>
    </div>
  )
