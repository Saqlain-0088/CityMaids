          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-2xl border border-slate-100 shadow-soft p-5">
              ...
              <div className="space-y-3">
                {[
                  { label: 'Total Customers', value: customers.length, icon: 'Ã°Å¸â€˜Â¥', color: 'text-blue-600' },
                  { label: 'Active Staff', value: staffMembers.filter(s => s.status === 'active').length, icon: 'Ã¢Å“â€¦', color: 'text-primary-600' },
                  { label: 'Pending Reviews', value: reviews.filter(r => r.status === 'pending').length, icon: 'Ã¢Â­Â', color: 'text-yellow-600' },
                  { label: 'New Leads', value: leads.filter(l => l.status === 'new').length, icon: 'Ã°Å¸â€œÂ§', color: 'text-purple-600' },
                ].map(item => (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-white rounded-2xl border border-slate-100 shadow-soft">
            ...
            <div className="p-5 space-y-4">
              {[
                { text: 'New booking from Sarah Johnson', time: '5 min ago', icon: 'Ã°Å¸â€œâ€¹', color: 'bg-blue-100 text-blue-600' },
                { text: 'Carlos Rivera completed a job', time: '23 min ago', icon: 'Ã¢Å“â€¦', color: 'bg-primary-100 text-primary-600' },
                { text: 'New lead: Patricia Moore', time: '1 hour ago', icon: 'Ã°Å¸â€œÂ§', color: 'bg-purple-100 text-purple-600' },
                { text: 'Review submitted by Emily Chen', time: '2 hours ago', icon: 'Ã¢Â­Â', color: 'bg-yellow-100 text-yellow-600' },
                { text: 'Service updated: Deep Cleaning', time: '3 hours ago', icon: 'Ã°Å¸Â§Â¹', color: 'bg-slate-100 text-slate-600' },
              ].map((a, i) => (
